...
const ItemFood = ({
  ...
  const buttonSave = async () => {
    const data = `food_name=${selectUser.food_name}&description=${selectUser.description}&price=${selectUser.price}`;

    await axios
      .post(
        `https://cashewwww.000webhostapp.com/api/apiFood.php?op=update&id=${selectUser.id}`,
        data,
      )
      .then(res => {
  ...
  const handleDelete = async () => {
    await axios
      .post(
        `https://cashewwww.000webhostapp.com/api/apiFood.php?op=delete&id=${selectUser.id}`,
      )
      .then(res => {
  ...
  const session_foodName = async () => {
    try {
      let keys = [
        ['id_food', selectUser.id],
        ['food_name', selectUser.food_name],
      ];
  ...
  if (statusInput === 'show') {
    return (
      <View style={styles.styleWrapItem}>
        ...
        <TouchableOpacity onPress={() => SelectUser(getFood)}>
        ...
        <View style={styles.styleWrapItem}>
          ...
          <View style={styles.styleParentsText}>
            <TextInput
              ...
              value={selectUser.food_name}
              onChangeText={value => onInputChange(value, 'food_name')}
              onFocus={() => SelectUser(getFood)}
              maxLength={20}
...
const EditFoods = ({navigation}) => {
  ...
  useEffect(() => {
    ...
    const removeSession = async () => {
      try {
        await AsyncStorage.removeItem('food_name');
      } catch (err) {
        console.log(err);
      }
    };
  ...
  return (
    <View style={styles.Container}>
          ...
          <View style={styles.styleContainItem}>
            <View style={styles.styleInput}>
              <Picker
                ...
                itemStyle={{fontSize: 90, color: 'blue'}}>
                {(Category || []).map(item => {
                  if (item.menu === 'food') {
      ...
      <TouchableOpacity
        ...
        onPress={() => navigation.navigate('AddingFood')}>
