...
const AddingFood = ({navigation}) => {
  ...
  }, [succesAdding, succesEdit, succesDelete, getAllNotif]);
  ...
  const handleAddCategory = () => {
    const data = `username=${username}&email=${email}&menu=food&category=${myCategory.category}`;
  ...
  const handleEditCategory = () => {
    ...
    (itemCategory || []).map(item => {
      const data = `category=${myCategory.category}`;
      if (item.category === dataFood.category) {
        axios
          .post(
            `https://cashewwww.000webhostapp.com/api/apiFood.php?op=update&id=${item.id}`,
            data,
          )
          .then(res => {
            console.log(`succes :${res}`);
  ...
  const dataSessionFood = async item => {
    try {
      await AsyncStorage.setItem('food_name', item);
    } catch (err) {
      console.log(err);
    }
  };
  ...
  const handleButtonSend = () => {
    const data = `username=${username}&email=${email}&food_name=${dataFood.food_name}&description=${dataFood.description}&price=${dataFood.price}&category=${dataFood.category}`;
    ...
    if (
      ...
      axios
        .post(
          `https://cashewwww.000webhostapp.com/api/apiFood.php?op=create`,
          data,
        )
        .then(res => {
          console.log('succes');
          dataSessionFood(dataFood.food_name);
          setDataFood({
            food_name: '',
            description: '',
            price: 0,
          ...
          navigation.navigate('EditPhotoFood');
        })
        .catch(err => {
          console.log(err);
        });
      ...
      <View style={styles.styleMain}>
        <View style={styles.styleContainerInput}>
          <View style={styles.styleParentsInput}>
            <Text style={styles.styleTextInput}>Food Name</Text>
            <TextInput
              style={styles.styleInput}
              onChangeText={value => onInputChange(value, 'food_name')}
              placeholderTextColor="grey"
            />
          </View>
          <View style={styles.styleParentsInput}>
          ...
          <View style={styles.styleParentsInput}>
            ...
            <View style={styles.styleInput}>
              <TouchableOpacity>
                <Picker
                  selectedValue={dataFood.category}
