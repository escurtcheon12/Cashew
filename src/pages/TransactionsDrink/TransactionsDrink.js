...
const ItemDrink = ({image, drink_name, description, price, getDrink, item}) => {
  const globalState = useSelector(state => state);
  const [number, setNumber] = useState(1);
  const [statusSession, setStatusSession] = useState('show');
  ...
  const handleOpenShow = () => {
    dispatch(
      addInventory({
        id: getDrink.id,
        menu: getDrink.drink_name,
        category: 'drink',
        item: 1,
        price: getDrink.price,
      }),
    );
    setStatusSession('edit');
  };

  const increseInventory = number => {
    dispatch(increaseItem(getDrink));
    setNumber(number + 1);
  };

  const decreaseInventory = number => {
    dispatch(decreaseItem(getDrink));
    if (number > 1) setNumber(number - 1);
  };

  const handleDeleteInventory = () => {
    dispatch(deleteInventory(getDrink.id));
    setStatusSession('show');
  };
  ...
  const Starts = () => {
    if (formulaRating() < 10) {
      return <Star style={styles.styleStarShow} height={9} width={9} />;
    } else if (formulaRating() >= 10 && formulaRating() <= 30) {
      return (
        <View style={styles.styleRating}>
          <Star style={styles.styleStarShow} height={9} width={9} />
          <Star style={styles.styleStarShow} height={9} width={9} />
        </View>
      );
        ...
        <View style={styles.styleParentsText}>
          ...
          <View style={styles.styleParentsMiddleShow}>
            ...
            <TouchableOpacity
              ...
              onPress={() => handleOpenShow(getDrink)}>
...
const TransactionsDrink = ({navigation}) => {
  const globalState = useSelector(state => state);
...
export default TransactionsDrink;
