...
const EditPhotoDrink = ({navigation}) => {
  ...
  useEffect(() => {
    const getDataDrink = async () => {
      try {
        const value = await AsyncStorage.getItem('drink_name');
        setDataDrink(value);
        setLoading(false);
      } catch (err) {
        console.log(err);
    ...
    const getDrink = async () => {
      await axios
        ...
        .then(res => {
          setAllDrink(res.data.data.result);
          setLoading(false);
        })
        .catch(err => {
          console.log(err);
  ...
  const handleChoosePhoto = option => {
    let i = search(dataDrink, allDrink);
    setIdDrink(i.id);
  ...
  const placeUpload = async () => {
    RNFetchBlob.fetch(
      'POST',
      `https://cashewwww.000webhostapp.com/api/apiDrink.php?op=uploadDrink&id=${idDrink}`,
      {
  ...
  let search = function (nameKey, myArray) {
    for (var i = 0; i < myArray.length; i++) {
      if (myArray[i].drink_name === nameKey) {
        return myArray[i];
      }
    }
  };
